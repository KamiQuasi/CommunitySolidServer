...
describe('A PermissionBasedAuthorizer', (): void => {
  ...
  beforeEach(async(): Promise<void> => {
    input = {
      ...
      permissionMap: new IdentifierMap(),
      credentials: {},
    };
    resourceSet = {
  ...
  it('allows access if the permissions are matched by the reader output.', async(): Promise<void> => {
    input.accessMap = new IdentifierMap([[ identifier, new Set([ AccessMode.read, AccessMode.write ]) ]]);
    input.permissionMap = new IdentifierMap([[ identifier, {
  ...
  it('throws a 404 in case the target resource does not exist and would not be written to.', async(): Promise<void> => {
    ...
    input.accessMap = new IdentifierMap([[ identifier, new Set([ AccessMode.delete ]) ]]);
    input.permissionMap = new IdentifierMap([[ identifier, {
  ...
  it('throws a ForbiddenHttpError if only some identifiers are authorized.', async(): Promise<void> => {
    ...
    input.accessMap = new IdentifierMap([
      [ identifier, new Set([ AccessMode.read, AccessMode.write ]) ],
      [ identifier2, new Set([ AccessMode.read, AccessMode.write ]) ],
    ]);
    input.permissionMap = new IdentifierMap([
      [ identifier, {
        [CredentialGroup.public]: { read: true, write: false },
        [CredentialGroup.agent]: { write: true },
      }],
